...
export function seedDatabase(): Database {
  ...
  const needs: Need[] = [
    ...
    },
    {
      ...
      resolution: {
        resolution: 'solved',
        note: 'Basement is dry and the furnace man could get in the next morning.',
        recordedBy: 'mem_hansen',
        onBehalfOfRequester: false,
        recordedAt: at(lastWeek, 2, 14),
      },
    },
    {
      id: 'need_snow',
      title: 'Clear a drifted driveway before a dialysis run',
      rawText:
        'The driveway drifted shut overnight and I have dialysis in Missoula Tuesday morning. I need it open by Monday night.',
      requesterId: 'mem_doris',
      postedById: null,
      neighborhood: 'Hamilton South',
      street: '412 S 3rd St',
      ...
      urgency: 'urgent',
      status: 'unmet',
      createdAt: at(lastWeek, 0, 8),
      neededBy: at(lastWeek, 1, 18),
      taskIds: ['task_snow_clear'],
      publishConsent: true,
    },
    {
      id: 'need_hay',
      title: 'Haul hay to stranded cattle off Skalkaho',
      rawText:
        'Road drifted in and the cattle on the back pasture are out of feed. Two round bales, needs a truck and a trailer.',
      requesterId: 'mem_hansen',
      postedById: null,
      neighborhood: 'Corvallis',
      street: '1140 Eastside Hwy',
      ...
      urgency: 'soon',
      status: 'open',
      createdAt: at(lastWeek, 2, 7),
      neededBy: at(lastWeek, 4, 17),
      taskIds: ['task_hay_haul'],
      publishConsent: true,
    },
    {
      id: 'need_crew_move',
  ...
  const commitments: Commitment[] = [
    ...
    // Ellis said yes to the hay haul; a second truck never came, so it never launched.
    c('cmt_10', 'mem_ellis', 'task_hay_haul', 'need_hay', at(lastWeek, 2, 19), false, 'committed'),
  ];
